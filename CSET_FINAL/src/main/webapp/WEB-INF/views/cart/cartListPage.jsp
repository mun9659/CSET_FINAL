...
function formatNumber(num) { // 변화되는 숫자에 천의 자리 마다 , 붙이기
	var regexp = /\B(?=(\d{3})+(?!\d))/g; // B는 단어 경계(띄어쓰기)가 아닌 부분에 대응 (ex. 010 0000 0000 에서 010|0000|0000 은 \b, 0|1|0 은 \B)
	 									   // 숫자 3개 뒤에 숫자가 없는 경우에만 일치시킨다. 즉 3자리 마다 , 을 집어넣을 수 있게 하는 정규식이다.
