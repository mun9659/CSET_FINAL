...
public class ReviewsPageMaker {


	public static String getPageView(String path, int pNo, int page, int recordPerPage, int totalRecord) {
		...
		if ( beginPageOfBlock < pagePerBlock ) {
			...
			sb.append("<a href='" + path + "?pNo=" + pNo + "&page=" + (beginPageOfBlock - 1) + "'><i class='fas fa-angle-left'></i></a>&nbsp;&nbsp;");
		}
		...
		for ( int p = beginPageOfBlock; p <= endPageOfBlock; p++ ) {
			if ( p == page ) {
				...
				sb.append("<a href='" + path + "?pNo=" + pNo + "&page=" + p + "'>" + p + "</a>&nbsp;&nbsp;");
			}
		}
		...
		if ( endPageOfBlock == totalPage ) {
			sb.append("<span><i class='fas fa-angle-right' style='color: lightgray;'></span>");
		} else {
			sb.append("<a href='" + path + "?pNo=" + pNo + "&page=" + (endPageOfBlock + 1) + "'><i class='fas fa-angle-right'></a>");
		}
		sb.append("</div>");
